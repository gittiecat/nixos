...
- Overwatch 2 game servers run on Google Cloud, each region in known IPv4
  ranges (e.g. NA West includes `24.105.8.0-24.105.15.255` plus many GCP
  `34.x`/`35.x` blocks).
- The matchmaker routes you to the **lowest-latency reachable** server. Block
  every *other* region (EU, NA East/Central, ME, AU, Japan, Korea, Singapore,
  Taiwan, Brazil) and it falls back to the one you left open.
- **Why it's scoped to the game:** the upstream lists cover whole Google Cloud
  regions (big `/14` blocks), and Discord's voice servers are on GCP too. A
  machine-wide block would kill Discord. Instead, the game runs inside a
...
| `ow-region metrics` | Write per-server drop counts to `/tmp/ow-region/metrics.txt`. |

**Region keys:** `na-west`, `na-east`, `na-central`, `eu`, `me`, `au`,
`japan`, `korea`, `singapore`, `taiwan`, `brazil`.
...
- **IP lists are fetched live** on each launch from the
  [foryVERX/Overwatch-Server-Selector](https://github.com/foryVERX/Overwatch-Server-Selector)
  repo (`raw.githubusercontent.com/.../ip_lists/Ip_ranges_<Region>.txt`), the
  same data the popular Windows tool uses. Needs network when the game starts.
...
- **`dropstats`** is a third set — `flags dynamic; counter` — so each dropped
  destination IP gets its own packet/byte counter (per-server drop metrics). The
  `metrics` subcommand snapshots it to `/tmp/ow-region/metrics.txt`, and teardown
  dumps it there before flushing the table.
- Set elements come straight from the upstream lists, extracted with `grep -oE`
  so multiple-ranges-per-line are handled. nftables interval sets accept both
  CIDR (`34.76.0.0/14`) and dash ranges (`24.105.8.0-24.105.15.255`).
- **Steam runtime env:** Steam injects an `LD_LIBRARY_PATH`/`LD_PRELOAD` pointing
  at its own runtime, which overrides our nixpkgs tools' RPATH and breaks them
  (e.g. `curl` loads Steam's incompatible `libcurl`). The script stashes those
  vars, scrubs them for its own work, and restores them only for the game launch
  (via an `env` wrapper inside the scope) so Proton/MangoHud still get them.
...
- **Lists can change upstream.** Fetched live, so a Blizzard server move just
  needs the foryVERX repo to update. If forcing stops working, check there first.
